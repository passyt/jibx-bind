 ...
 *
 */
public class CollectionXmlParser extends AbstractXmlParser {

	protected CollectionXmlParser(IXMLReader reader) {
		super(reader);
	}

	@SuppressWarnings("unchecked")
	public CollectionElement parse(MappingElement mappingElement, ValueStyle defaultValueStyle) throws JiBXException {
		...
		for (int i = 0; i < attributeCount; i++) {
			...
			if ("add-method".equals(attributeName)) {
				element.setAddMethodName(attributeValue);
			} else if ("item-type".equals(attributeName)) {
				element.setItemTypeName(attributeValue);
			} else if ("iter-method".equals(attributeName)) {
				element.setIterMethodName(attributeValue);
			} else if ("load-method".equals(attributeName)) {
				element.setLoadMethodName(attributeValue);
			} else if ("size-method".equals(attributeName)) {
				element.setSizeMethodName(attributeValue);
			} else if ("store-method".equals(attributeName)) {
				element.setStoreMethodName(attributeValue);
			} else {
				boolean found = false;
